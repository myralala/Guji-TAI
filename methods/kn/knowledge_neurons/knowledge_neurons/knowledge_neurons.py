...
###change
        if self.model_type == "bert":
            ...
            if mask_positions.nelement() == 0:
                ...
                encoded_input["input_ids"] = torch.tensor([input_ids]).to(self.device)

                if "attention_mask" in encoded_input:
                    attention_mask = encoded_input["attention_mask"][0].tolist()
                    attention_mask.append(1)
                    encoded_input["attention_mask"] = torch.tensor([attention_mask]).to(self.device)

                if "token_type_ids" in encoded_input:
                    token_type_ids = encoded_input["token_type_ids"][0].tolist()
                    token_type_ids.append(0)
                    encoded_input["token_type_ids"] = torch.tensor([token_type_ids]).to(self.device)
        ...
        if target is not None:
            if "qwen" in self.model_type or "gpt" in self.model_type or 't5' in self.model_type or 'llama' in self.model_type or "chatglm" in self.model_type:
    ...
    def get_scores_for_layer(
        ...
        # for autoregressive models, we might want to generate > 1 token
        n_sampling_steps = len(target_label) if ("qwen" in self.model_type or "gpt" in self.model_type or 'llama' in self.model_type or 'chatglm2'  in self.model_type) else 1
