...
class Patch(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = self.ff(x)
        if self.mode == "replace":
            replacement = self.acts.to(dtype=x.dtype)
            if x.shape[0] == replacement.shape[0]:
                x[:, self.mask_idx, :] = self.acts.to(dtype=x.dtype)
            elif x.dim() >= 3 and x.shape[1] == replacement.shape[0]:
                x[self.mask_idx, :, :] = replacement
            else:
                # Fallback for architectures whose FF blocks expose the batch axis in
                # an unexpected position. We only transpose when it makes the batch
                # dimension align with the replacement activations.
                transposed = x.transpose(0, 1)
                if transposed.shape[0] == replacement.shape[0]:
                    transposed[:, self.mask_idx, :] = replacement
                    x = transposed.transpose(0, 1)
                else:
                    raise RuntimeError(
                        f"Patch shape mismatch: hidden={tuple(x.shape)} replacement={tuple(replacement.shape)} "
...
def temporary_patch_ff_layer(
    ...
    try:
        ...
        elif mode in ["suppress", "enhance"] and neurons:
            layer_indices = sorted({layer for layer, _ in neurons})
            unpatch_ff_layers(
                model,
