...
def find_token_range(tokenizer, token_array, substring):
    ...
    try:
        ...
        try:
            ...
            # Graceful fallback: use the beginning span when exact match is not found.
            fallback_len = min(max(2, len(substring)), max(1, len(toks) // 4))
...
def plot_trace_heatmap(result, savepdf=None, title=None, xlabel=None, modelname=None):
    ...
    with chinese_plot_context():
        plot_meta = causal_trace_plot_meta(kind=kind, modelname=modelname or "GPT", window=window)
        ...
        ax.set_title(plot_meta["title"])
        ax.set_xlabel(plot_meta["x_label"])
        cb = plt.colorbar(h)
