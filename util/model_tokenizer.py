...
def _log_model_loading(model_name, model_path, stream=None):
    print(f"loading model: {model_name} from path: {model_path}", file=stream or sys.stderr)


def _log_model_loaded(model_name, stream=None):
    print(f"{model_name} model loaded", file=stream or sys.stderr)


def _ensure_internlm_losskwargs_compat():
...
def initialize_model_and_tokenizer(model_name: str, model_path: str =None, dtype=torch.float16, device_map="auto"):
    ...
    if 't5' in model_name.lower():
        ...
        _ensure_internlm_losskwargs_compat()
...
class ModelAndTokenizer:
    ...
    def __init__(
        ...
        _log_model_loading(model_name, model_path)
    ...
    with lock_get_cached_model_tok:
        if model_name not in model_name2obj:
            if (model2path is not None) and (model_path is None):
                resolved_model_path = model2path.get(model_name, model_name)
