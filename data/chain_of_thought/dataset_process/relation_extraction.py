...
from data.chain_of_thought.dataset_process.guji_common import normalize_relation_extraction_sample, resolve_repo_data_path
...
class GujiStructDataset(Dataset):
    def __init__(self, loc, domain="traditional_culture"):
        ...
        self.data = [
            normalize_relation_extraction_sample(x, domain=self.domain)
...
def get_processed_kvs(sample, keys=None, required_keys=None):
    item = {
        ...
        "sub_task": sample.get("sub_task", "relation_extraction"),
...
def get_default_dataset():
    return GujiStructDataset(
        loc=resolve_repo_data_path("0_shot", "relation_extraction.json"),
